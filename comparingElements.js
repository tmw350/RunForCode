// Note: We can initialize an array bu pitting the list of elements in square brackets.
// Also note that JaveScript is loosely typed [ which means it allows us to mix types in our arrays...
// And that can cause all sorts of confusion
// Mixing elememnt types like this is a "bad" practice
...
for (var arrayPosition = 0;
	arrayPosition < testArray.length -1; arrayPosition++){
		...
		if(currentElement == nextElement){
			// If they are == to each other, are they strictly equal (===) to each other?
			console.log("Testing " + currentElement + " and " + nextElement + "(strictly equal to)" + (currentElement === nextElement))
			;
			
			// if they are not strictly equal (===), what is their type?
			if(currentElement !== nextElement)
			{
